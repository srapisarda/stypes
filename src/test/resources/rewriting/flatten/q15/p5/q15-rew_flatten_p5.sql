WITH p28 AS (SELECT s_0.x AS X0, r_1.y AS X1
     ...
     p2 AS (SELECT r_0.x AS X0, p14_1.X1 AS X1
            FROM r AS r_0
                     INNER JOIN p14 AS p14_1 ON r_0.y = p14_1.X0
                     INNER JOIN b AS b_2 ON r_0.x = b_2.x
            UNION
            (SELECT r_2.x AS X0, p14_3.X1 AS X1
             FROM r AS r_0
                      INNER JOIN s AS s_1 ON r_0.x = s_1.y
                      INNER JOIN r AS r_2 ON s_1.x = r_2.y
                      INNER JOIN p14 AS p14_3 ON r_0.y = p14_3.X0)
            UNION
