WITH p7 AS (SELECT s_0.x AS X0, r_1.y AS X1
            FROM s AS s_0
     ...
     p19 AS (SELECT r_0.x AS X0, r_0.y AS X1
             ...
             (SELECT r_0.x AS X0, s_2.y AS X1
              FROM r AS r_0
                       ...
                       INNER JOIN s AS s_2 ON r_1.y = s_2.x)),
     p3 AS (SELECT r_0.x AS X0, r_4.y AS X1
            FROM r AS r_0
                     INNER JOIN r AS r_1 ON r_0.y = r_1.x
                     INNER JOIN s AS s_2 ON r_1.y = s_2.x
            ...
            UNION
            (SELECT r_0.x AS X0, r_1.y AS X1
             FROM r AS r_0
                      INNER JOIN r AS r_1 ON r_0.y = r_1.x
                      INNER JOIN a AS a_2 ON r_1.y = a_2.x)
            UNION
            (SELECT p19_0.X0 AS X0, r_3.y AS X1
             FROM p19 AS p19_0
                      INNER JOIN r AS r_1 ON p19_0.X1 = r_1.x
                      INNER JOIN s AS s_2 ON r_1.y = s_2.x
                      INNER JOIN r AS r_3 ON s_2.y = r_3.x
                      INNER JOIN a AS a_4 ON r_3.y = a_4.x)
            UNION
            (SELECT p19_0.X0 AS X0, r_1.y AS X1
             FROM p19 AS p19_0
                      INNER JOIN r AS r_1 ON p19_0.X1 = r_1.x
                      INNER JOIN a AS a_2 ON r_1.y = a_2.x)
            UNION
